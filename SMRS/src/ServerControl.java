...
class ServerControl {
    ...
    synchronized UserDescription findUser (String uuid){
    ...
    synchronized UserDescription findUserByID (int id){
        UserDescription user = null;
        for (UserDescription u: users) {
                if (id == u.id) {
                    user = u;
    ...
    String
    sendMessage ( int src, int dst, String msg, String receipt, String sign, String sk ) {
        int nr = 0, ns = 0, nsk = 0;
        String result;
        String path = null;
        ...
        try {
            ...
            ns = newFile( path, "s" + src + "_" );
            saveOnFile ( path + "s" + src + "_" + ns, sign );
            nsk = newFile( path, "sk" + src + "_" );
            saveOnFile ( path + "sk" + src + "_" + nsk, sk );
    ...
    String readSignFile ( int id, String msg ) throws Exception {
        String path = userMessageBox( id ) + "/";
        msg = "s" + msg;
        if (msg.charAt( 0 ) == '_') { // Already red
            path += msg;
    ...
    String readSKFile ( int id, String msg ) throws Exception {
        String path = userMessageBox( id ) + "/";
        msg = "sk" + msg;
        if (msg.charAt( 0 ) == '_') { // Already red
            path += msg;
    ...
    recvMessage ( int id, String msg ) {
        ...
        try {
            ...
            result += "\"" + readSignFile( id, msg ) + "\",";
            result += "\"" + readSKFile( id, msg ) + "\"";
        } catch(Exception e) {
